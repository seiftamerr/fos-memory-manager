...
void __page_fault_handler_with_buffering(struct Env *curenv, uint32 fault_va)
{
    ...
    if (env_page_ws_get_size(curenv) < curenv->page_WS_max_size)
    {
        ...
        {
            frame_info = get_frame_info(curenv->env_page_directory, (void*)fault_va, &page_table);
        ...
        else
        {
            ...
            if (ret == E_PAGE_NOT_EXIST_IN_PF) {
                if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP) {
                    ...
                    cprintf("Invalid access to VA %x - terminating environment\n", fault_va);
    ...
    {
        uint32 victim_va = 0;
        uint32 victim_idx = curenv->page_last_WS_index;
        uint32 permissions;

        while (victim_va == 0) {
            uint32 va = env_page_ws_get_virtual_address(curenv, victim_idx);
            permissions = pt_get_page_permissions(curenv, va);

            if (!(permissions & PERM_USED) && !(permissions & PERM_MODIFIED)) {
                victim_va = va;
                break;
            }

            victim_idx = (victim_idx + 1) % curenv->page_WS_max_size;

            if (victim_idx == curenv->page_last_WS_index) {
                do {
                    va = env_page_ws_get_virtual_address(curenv, victim_idx);
                    permissions = pt_get_page_permissions(curenv, va);
                    ...
                    victim_idx = (victim_idx + 1) % curenv->page_WS_max_size;
                } while (victim_idx != curenv->page_last_WS_index);
            }
        }

        frame_info = get_frame_info(curenv->env_page_directory, (void*)victim_va, &page_table);
        ...
        if (pt_get_page_permissions(curenv, victim_va) & PERM_MODIFIED) {
            ...
            if (LIST_SIZE(&modified_frame_list) >= getModifiedBufferLength()) {
                struct Frame_Info *modified_frame;
                LIST_FOREACH(modified_frame, &modified_frame_list) {
                    pf_update_env_page(modified_frame->environment, (void*)modified_frame->va, modified_frame);
                    pt_set_page_permissions(modified_frame->environment, modified_frame->va, 0, PERM_MODIFIED);
                    bufferlist_remove_page(&modified_frame_list, modified_frame);
                    bufferList_add_page(&free_frame_list, modified_frame);
                }
            }
        } else {
    ...
    tlb_invalidate(curenv->env_page_directory, (void*)fault_va);
}
