...
void __env_free_with_buffering(struct Env *e)
{
    // [1] Un-buffer any BUFFERED pages belonging to this environment from free and modified lists
    struct Frame_Info *frame_info, *next_frame_info;

    // Modified frames list
    LIST_FOREACH(frame_info, &modified_frame_list)
    {
        next_frame_info = LIST_NEXT(frame_info);
        if (frame_info->environment == e)
            LIST_REMOVE(&modified_frame_list, frame_info);
    }

    // Free frames list
    LIST_FOREACH(frame_info, &free_frame_list)
    {
        next_frame_info = LIST_NEXT(frame_info);
        if (frame_info->environment == e)
    ...
    // [4] Free all empty TABLES from the main memory
    uint32* pd = e->env_page_directory;
    for (uint32 pd_idx = 0; pd_idx < PDX(USER_TOP); pd_idx++)
    {
        if (pd[pd_idx] & PERM_PRESENT)
        {
            uint32* pt = (uint32*)(EXTRACT_ADDRESS(pd[pd_idx]) + KERNEL_BASE);

            int empty = 1;
            for (uint32 pt_idx = 0; pt_idx < 1024; pt_idx++)
            {
                if (pt[pt_idx] & PERM_PRESENT)
                {
                    empty = 0;
                    break;
                }
            ...
            if (empty)
            {
                uint32 pt_va = pd_idx << 22;
                unmap_frame(pd, (void*)pt_va);
            }
        }
    }
