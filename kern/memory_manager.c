 ...
 */
 ...
 void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
 {
     ...
     // Round down the virtual address to be aligned with page boundary
     virtual_address = ROUNDDOWN(virtual_address, PAGE_SIZE);

     // Calculate the number of pages needed
     int num_pages = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;

     // validation on virtual_address range
     if (virtual_address + (num_pages * PAGE_SIZE) > USER_HEAP_MAX)
         panic("error");

     // For each page in the range:
     for (int i = 0; i < num_pages; i++) {
         uint32 curr_va = virtual_address + (i * PAGE_SIZE);

         // Check if page is already allocated in page table
         uint32 *ptr_page_table = NULL;
         struct Frame_Info* frame_info = get_frame_info(e->env_page_directory, (void*)curr_va, &ptr_page_table);

                  if (frame_info != NULL) {
                      // Page is already allocated - this is an error condition
                      panic("ERROR: allocateMem() - Page already allocated");
                  }

         // Add the page to the page file
         int ret = pf_add_empty_env_page(e, curr_va, 0);

         // Check if adding to page file succeeded
         if (ret == E_NO_PAGE_FILE_SPACE) {
             // Free all pages allocated so far in case of failure
             for (int j = 0; j < i; j++) {
                 uint32 va_to_free = virtual_address + (j * PAGE_SIZE);
                 pf_remove_env_page(e, va_to_free);
             }
             panic("ERROR: allocateMem() failed - No enough space in the page file");
         }
     }
 }
 ...
 void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
 {
     ...
     // Calculate the number of pages needed to be freed
     int num_pages = ROUNDUP(size, PAGE_SIZE) / PAGE_SIZE;
     uint32 va_end = virtual_address + num_pages * PAGE_SIZE;

     // Iterate over all pages in the range
     for (uint32 va = virtual_address; va < va_end; va += PAGE_SIZE)
     {
         // 1. Handle BUFFERED pages and pages in working set
         uint32 *ptr_page_table;
         struct Frame_Info *frame_info = get_frame_info(e->env_page_directory, (void*)va, &ptr_page_table);

         if (frame_info != NULL && ptr_page_table != NULL)
         {
             // Check if the page is buffered
             if (pt_get_page_permissions(e, va) & PERM_BUFFERED)
             {
                 // Check if the page is modified
                 if (pt_get_page_permissions(e, va) & PERM_MODIFIED)
                 {
                     // Remove it from the modified_frame_list
                     bufferlist_remove_page(&modified_frame_list, frame_info);

                     // Flag it as a free frame
                     frame_info->isBuffered = 0;
                     frame_info->environment = NULL;
                     frame_info->va = 0;

                     // Add it to the head of free_frame_list
                     free_frame(frame_info);
                 }
                 else
                 {
                     // Just flag it as a free frame
                     frame_info->isBuffered = 0;
                     frame_info->environment = NULL;
                     frame_info->va = 0;

                     // Remove frame from free_frame_list and bring it to the head
             ...
             // 2. Handle pages in the working set
             else
             {
                 // Find the page in the working set
                 for (int i = 0; i < e->page_WS_max_size; i++)
                 {
                     if (!env_page_ws_is_entry_empty(e, i) &&
                         ROUNDDOWN(e->ptr_pageWorkingSet[i].virtual_address, PAGE_SIZE) == va)
                     {
                         // Clear the entry in the page table
                         pt_clear_page_table_entry(e, va);

                         // Clear the entry in the working set
         ...
         // 3. Free the page from the page file
         pf_remove_env_page(e, va);
     }

     // 4. Remove EMPTY page tables in the given range
     // Iterate over the address range by page directory entries
     uint32 dir_start = PDX(virtual_address);
     uint32 dir_end = PDX(va_end - 1);

     for (uint32 dir_idx = dir_start; dir_idx <= dir_end; dir_idx++)
     {
         // Get the first virtual address for this directory entry
         uint32 dir_va = dir_idx << 22;

         // Check if this directory entry is present
         if (e->env_page_directory[dir_idx] & PERM_PRESENT)
         {
             // Get the pointer to the page table
             uint32 *ptr_page_table = NULL;
             get_page_table(e->env_page_directory, (void*)dir_va, &ptr_page_table);

             if (ptr_page_table != NULL)
             {
                 // Check if all entries in this page table are empty
                 bool is_table_empty = 1;

                 for (int pt_idx = 0; pt_idx < 1024; pt_idx++)
                 {
                     if (ptr_page_table[pt_idx] & PERM_PRESENT)
                     {
                         is_table_empty = 0;
                         break;
                     }
                 ...
                 if (is_table_empty)
                 {
                     // Free the frame that holds the page table
                     struct Frame_Info *pt_frame = to_frame_info(e->env_page_directory[dir_idx] & 0xFFFFF000);
                     free_frame(pt_frame);

                     // Clear the page directory entry
                     pd_clear_page_dir_entry(e, dir_va);
                 }
             }
         }
     }
